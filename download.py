...
def process_image(job):
    ...
    if not os.path.isfile((job.get("job_folder")+job.get("img_file")).split(".")[0]+".lz4"):
        urllib.request.urlretrieve(job.get("url"), job.get("img_folder")+job.get("img_file"))
    t = Image.open(job.get("img_folder")+job.get("img_file"))
    #use quantize functions instead of PIL quantize
     
    
    out = t.convert("RGB").filter(ImageFilter.EDGE_ENHANCE)#.quantize(palette=img_pal)
    ...
    out.save(job.get("img_folder")+job.get("img_file").replace(".png","_dt.png"))

    z = open((job.get("job_folder")+job.get("img_file")).split(".")[0]+".lz4", "w+b")
    z.write(lz4.frame.compress(bytearray(epd.getbuffer(out))))
    z.close()
    
    r = open((job.get("job_folder")+job.get("img_file")).split(".")[0]+".raw", "w+b")
    r.write(bytearray(epd.getbuffer(out)))
    r.close()
    print("({0:1.2f}%) {1} -> {2}".format(percent, job.get("url"),job.get("img_folder")+job.get("img_file")))        
...
def get_jobs_for(lon,lat,zoom): 
    ...
    print('Tiles: ({4}/{0}/{1}.png) -> ({4}/{2}/{3}.png)'.format(x[0],y[0],x[1],y[1],zoom))
    jobs=[]
    for dx in range(abs(x[1]-x[0])) :
        for dy in range(abs(y[1]-y[0])):
            ...
            jobs.append({"url":url,"job_folder":job_folder,"img_folder":img_folder,"img_file":img_file})
